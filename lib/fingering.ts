...
export function assignFingers(notes: Note[], count: number): FingerNote[] {
  if (!Number.isInteger(count) || count < 1 || count > 5)
    throw new Error('Finger count must be between 1 and 5.');
  ...
  for (const hand of ['left', 'right'] as const) {
    ...
    for (const start of new Set(own.map((n) => n.start)))
      if (own.filter((n) => n.start === start).length > count)
        throw new Error(
          `Score exceeds ${count} simultaneous fingers on the ${hand} hand at ${start.toFixed(2)}s.`,
        );
    for (const start of [...new Set(own.map((n) => n.start))]) {
      ...
      if (chord.length + held.length > count) {
        ...
        const release = held
          .slice()
          .sort((a, b) => a.start - b.start || a.midi - b.midi)
          .slice(0, chord.length + held.length - count);
        release.forEach((n) => {
      ...
      function assign(attacks: Note[], fixed: FingerNote[]) {
        return optimizeFingers(
          [...attacks, ...own.filter((n) => n.start > start).slice(0, 14)],
          count,
          fixed,
          previous,
        );
      ...
      if (!fingers.length) {
        attacks = [
          ...held.map((n) => ({ ...n, start, duration: n.start + n.duration - start })),
          ...chord,
      ...
      attacks.forEach((n, i) => {
        result.push({ ...n, finger: fingers[i] });
      });
      previous = {
        ...attacks[attacks.length - 1],
        finger: fingers[fingers.length - 1],
        chord: attacks.length > 1,
      };
    }
  }
  ...
  for (const n of result) {
    const next = result.find(
      (m) => m.hand === n.hand && m.start > n.start && (m.finger === n.finger || m.midi === n.midi),
    );
    if (!next || next.start > n.start + n.duration + 0.08) continue;
    if (
      next.midi === n.midi &&
      !notes.some((m) => m.hand === next.hand && m.midi === next.midi && m.start === next.start)
    )
      continue;
    const gap = Math.min(0.08, (next.start - n.start) * 0.25);
