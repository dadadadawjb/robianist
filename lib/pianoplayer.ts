...
const weights=[1.1,1,1.1,.9,.8];
const blackBias=[.3,1,1.1,.8,.7];
const stretch:Record<string,number>={'2,3':5,'3,4':5,'1,2':6,'1,3':7,'2,4':8,'1,4':11,'0,1':12,'0,2':14,'0,3':16};
...
export function fingerVelocity(a:Note,fa:number,b:Note,fb:number):number {
  ...
  return Math.abs(x(b)-position)/(Math.abs(b.start-a.start)+.1)/weights[fb]/(isBlack(b.midi)?blackBias[fb]:1);
}
...
export function skipTransition(a:MotionNote,fa:number,b:MotionNote,fb:number):boolean {
  ...
  if(!a.chord&&!b.chord) {
    ...
    if(limit!==undefined&&Math.abs(dx)*handFactor/.8>limit)return true;
  }
  return false;
}
...
export function optimizeFingers(notes:Note[],count:number,held:Contact[],previous?:MotionNote & {finger:number}):number[] {
  ...
  for(let i=3;i<depth;i++)if(notes[i].start-notes[0].start>3.5){depth=i+1;break;}
  // Do not optimize a partial chord at the end of the window.
  while(depth>attacks&&depth<notes.length&&notes[depth-1].start===notes[depth].start)depth--;
  const window=notes.slice(0,depth).map((n,i)=>({...n,chord:notes[i-1]?.start===n.start||notes[i+1]?.start===n.start}));
  ...
  function solve(relax:boolean):Solution {
    ...
    function visit(i:number,fa:number):Solution {
      ...
      const b=window[i],a=i?window[i-1]:previous;
      let best:Solution={cost:Infinity,fingers:[]};
      for(let fb=0;fb<count;fb++) {
        if(i<attacks&&held.some(n=>n.finger===fb||(b.midi-n.midi)*(fb-n.finger)*(b.hand==='right'?1:-1)<=0))continue;
