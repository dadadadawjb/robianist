...
├── components/                  scene, grand piano, robot asset and retained live sheet music
...
│   ├── audio.ts                 sampled piano, speed-adjusted pedal sustain and bounded scheduling
...
│   └── score.ts                 MusicXML/MXL timing, arpeggios, dynamics, wedges and pedal events
├── public/
...
├── tests/                       asset, fingering, music, keyboard, IK and manual UI checks
├── .github/workflows/           GitHub Pages build and deployment automation
├── .prettierrc.json              shared readable code-formatting rules
...
- `app/page.tsx`: fixed configuration badge, score selection, dismissible MusicXML help, retained sheet panel and playback speed controls.
- `components/Scene.tsx`: metre-scale stage, piano bench with foot support camera views with manual override and a stage-ready signal for audio preloading.
- `components/SheetMusic.tsx`: retained OpenSheetMusicDisplay engraving, width-only relayout and visible-only audio-clock cursor.
- `tests/ui-checks.md`: sheet panel reuse, cursor/resize and MusicXML help dismissal regressions.
...
- `lib/score.ts`: validated MusicXML/MXL imports, ties, grace timing, tempo conversion, arpeggios, staff dynamics, paired hairpin wedges and pedal events.
...
- `lib/player.ts`: audio-clock playback, background sample preloading after stage readiness, synchronized playback speed, seeking and pause.
- `lib/audio.ts`, `tests/audio.test.ts`: smplr sampled piano scheduling, pedal-held tails, repeated attacks and seek/cleanup regressions.
- `lib/presets.ts`: fixed G1 + Wuji model, arm/finger joint descriptors and shared seated depth.
- `.prettierrc.json`, `package.json`: project formatting rules and `npm run format` / `npm run format:check` scripts.

## Running

- `npm install`, then `npm run dev`.
- `npm run format` to apply the shared style; `npm run format:check` to verify it.
