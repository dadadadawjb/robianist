...
test('concert pitch and semitone tuning', () => {
...
test('repeated keys and reused fingers release before the next attack without changing audio', () => {
  for (const midi of [60, 62])
    for (const interval of [0.12, 0.5]) {
      const source = [
        { midi: 60, start: 0, duration: interval, hand: 'right' as const },
        { midi, start: interval, duration: interval, hand: 'right' as const },
      ];
      const planned = assignFingers(source, 1);
      assert.ok(
        Math.abs(planned[0].duration - (interval - Math.min(0.08, interval * 0.25))) < 1e-9,
      );
      assert.equal(planned[1].start, interval);
      assert.equal(source[0].duration, interval);
    }
});
test('88-key keyboard maps all notes monotonically', () => {
  for (let m = 22; m <= 108; m++) assert.ok(keyX(m) > keyX(m - 1));
  assert.equal(isBlack(61), true);
  assert.equal(keyX(60), -2.5 * 0.025);
});
test('Human Light fits four and five fingers without overlapping assignments', () => {
  ...
  for (const count of [4, 5]) {
    ...
    for (const n of planned) {
      ...
      assert.ok(n.duration > 0 && n.start >= 0 && n.start + n.duration <= song.duration);
      for (const other of planned) {
        if (
          n !== other &&
          n.hand === other.hand &&
          n.start < other.start + other.duration - 1e-6 &&
          other.start < n.start + n.duration - 1e-6
        )
          assert.notEqual(n.finger, other.finger);
      }
    }
  }
});
test('too many simultaneous notes fail clearly', () => {
  assert.throws(
    () =>
      assignFingers(
        [60, 62, 64, 65, 67].map((midi) => ({ midi, start: 0, duration: 1, hand: 'right' })),
        4,
      ),
    /exceeds 4/,
  );
});
test('held notes retain fingers when a second voice enters', () => {
  const notes = assignFingers(
    [
      { midi: 60, start: 0, duration: 2, hand: 'right' },
      { midi: 64, start: 1, duration: 0.5, hand: 'right' },
    ],
    4,
  );
  assert.notEqual(notes[0].finger, notes[1].finger);
});
...
test('If Only plays with approximate contacts while preserving all audio notes', () => {
  ...
  for (const count of [4, 5]) {
    ...
    for (const note of song.notes)
      assert.ok(
        planned.some((n) => n.hand === note.hand && n.midi === note.midi && n.start === note.start),
        'Every attack has a finger contact',
      );
    for (const n of planned) {
...
test('substitution splits visual contact without retriggering the audio note', () => {
  const notes = [
    { midi: 60, start: 0, duration: 2, hand: 'right' as const },
    { midi: 55, start: 0, duration: 0.5, hand: 'right' as const },
    { midi: 62, start: 1, duration: 0.5, hand: 'right' as const },
  ];
  const planned = assignFingers(notes, 2);
  assert.ok(planned.some((n) => n.midi === 60 && n.start === 1));
  assert.equal(
    planned.filter((n) => n.midi === 60).reduce((sum, n) => sum + n.duration, 0),
    2,
  );
...
test('keyboard white-key pitch is 25 mm and spans 1.300 m', () => {
  assert.ok(Math.abs(keyX(23) - keyX(21) - 0.025) < 1e-12);
  assert.ok(Math.abs(keyX(108) - keyX(21) + 0.025 - 1.3) < 1e-12);
});
