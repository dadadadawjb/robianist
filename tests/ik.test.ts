...
test('URDF position IK reaches a piano target while respecting joint limits',()=>{
  ...
  const ik=createPoseIK(robot,'panda_link8',Array.from({length:7},(_,i)=>`panda_joint${i+1}`),[0,-.45,0,-1.9,0,1.5,.7]);const target=new THREE.Vector3(.43,.08,.44);
...
test('arbitrary local joint axes reach a target',()=>{
  ...
  const ik=createPoseIK(robot,'tip',['j0','j1'],[.2,.6]);const target=new THREE.Vector3(.3,0,.45);
