...
test('model URLs support root and GitHub Pages hosting without double prefixes', () => {
  ...
  try {
    ...
    assert.equal(
      assetUrl('/models/g1_wuji/g1_wuji.urdf'),
      '/robianist.js/models/g1_wuji/g1_wuji.urdf',
    );
    assert.equal(
      assetUrl('/models/vendor/hand/mesh.STL'),
      '/robianist.js/models/vendor/hand/mesh.STL',
    );
    for (const url of [
      '/robianist.js/models/g1_wuji/meshes/g1/pelvis.STL',
      'texture.png',
      'blob:example',
