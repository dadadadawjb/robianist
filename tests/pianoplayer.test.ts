...
const references = JSON.parse(
  readFileSync(new URL('./fixtures/pianoplayer.json', import.meta.url), 'utf8'),
) as { notes: Note[]; fingers: number[]; velocity: number }[];
...
test('lookahead prepares the thumb crossing in an ascending scale', () => {
  ...
  assert.deepEqual(
    assignFingers(notes, 5).map((n) => n.finger),
    fingers,
  );
});
test('upstream crossing, repetition, black-key and chord stretch rules', () => {
  const a = { midi: 60, start: 0, duration: 0.5, hand: 'right' as const, chord: false };
...
test('short inputs and outsize spans retain valid contacts without changing audio', () => {
  ...
  assert.deepEqual(
    assignFingers([{ midi: 60, start: 0, duration: 1, hand: 'right' }], 5).map((n) => n.finger),
    [0],
  );
  for (const hand of ['left', 'right'] as const) {
    ...
    assert.deepEqual(
      planned.map((n) => n.finger),
      hand === 'right' ? [0, 1, 2, 3, 4] : [4, 3, 2, 1, 0],
    );
