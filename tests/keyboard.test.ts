...
test('visible keys have metre-scale dimensions independent of piano shell scaling', () => {
  assert.equal(keyGeometry(60, false).length, 0.18);
  assert.equal(keyGeometry(61, false).length, 0.11);
  ...
  for (const midi of [60, 61])
    for (const pressed of [false, true]) {
      const key = keyGeometry(midi, pressed),
        [, y, z] = keyContact(midi, pressed);
      const dy = y - keyboard.pivotY,
        dz = z - keyboard.rearZ;
      ...
      if (pressed) {
        assert.ok(y < keyContact(midi, false)[1]);
        assert.ok(
          Math.abs(key.length * Math.sin(key.angle) - (isBlack(midi) ? 0.009 : 0.01)) < 1e-12,
        );
      }
    }
});
