...
export default function GrandPiano({plannedNotes,time,playing}:{plannedNotes:Note[];time:number;playing:boolean}) {
  ...
  return <group>
    ...
    {Array.from({length:88},(_,i)=>i+21).map(midi=>{const black=isBlack(midi);const active=playing&&plannedNotes.some(n=>n.midi===midi&&time>=n.start&&time<n.start+n.duration);return <group key={midi} position={[keyX(midi),1.2,-.82]} rotation={[active?.035:0,0,0]}><mesh position={[0,black?.1:0,black?.22:.42]} castShadow receiveShadow><boxGeometry args={[black?.057:.098,black?.12:.11,black?.48:.88]}/><meshStandardMaterial color={active?(black?'#333333':'#d6d6d6'):black?'#141414':'#f5f3ed'} roughness={.25}/></mesh></group>;})}
