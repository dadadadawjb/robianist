  ...
  if (!cache.has(url))
    cache.set(
      ...
      new Promise((resolve, reject) => {
        ...
        manager.onError = (p) => reject(new Error(`Model resource failed: ${p}`));
        manager.itemStart(url);
        fetch(assetUrl(url))
          .then((r) => {
            if (!r.ok) throw new Error(`Model ${r.status}: ${url}`);
            return r.text();
          })
          .then((xml) => {
            loader.workingPath = url.slice(0, url.lastIndexOf('/') + 1);
            model = loader.parse(xml);
            manager.itemEnd(url);
          })
          .catch(reject);
      }),
...
function useModel(url: string) {
  ...
  return useMemo(() => {
    const model = source.clone() as URDFRobot;
    model.traverse((o) => {
