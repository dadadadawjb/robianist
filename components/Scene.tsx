...
function Performer({side,...props}:Props & {side:Side}) {
  ...
  const notes=useMemo(()=>assignFingers(props.song.notes,props.hand.fingers).filter(n=>n.hand===side),[props.song,props.hand,side]);
...
export default function Scene(props:Props) {
  return <Canvas shadows={!props.lightweight} dpr={props.lightweight?1:[1,1.75]} camera={{position:[5,4.4,6.3],fov:39}} gl={{antialias:true}}>
    ...
    {!props.lightweight&&<ContactShadows position={[0,-.02,0]} opacity={.5} scale={16} blur={2.5} far={6} resolution={512}/>}
